...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "data_dir", help="Path to extracted Imagenet data downloaded from official website (https://image-net.org)"
    )
    ...
    shutil.copy2(
        os.path.join(args.data_dir, "ILSVRC2012_devkit_t12.tar.gz"),
        os.path.join(args.data_dir, "resized_imagenet/ILSVRC2012_devkit_t12.tar.gz"),
    )

    print(
        'Finished! You can now add "resized_imagenet" to your root path in the torchvision dataloader for using the resized version'
    )
